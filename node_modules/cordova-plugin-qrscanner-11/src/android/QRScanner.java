...
public void barcodeResult(BarcodeResult barcodeResult) {
    ...
    if (text != null && text.matches("[0-9]+")) {
        ...
        if (raw != null &&
    (raw.length == 16 || raw.length == 17 ||
     raw.length == 32 || raw.length == 34)) {

    if (raw.length == 17) {
        raw = java.util.Arrays.copyOfRange(raw, 1, raw.length);
    } else if (raw.length == 34) {
        raw = java.util.Arrays.copyOfRange(raw, 2, raw.length);
    }
    ...
    this.nextScanCallback.success(base64);

} else {

    this.nextScanCallback.success(text == null ? "NULL" : text);
