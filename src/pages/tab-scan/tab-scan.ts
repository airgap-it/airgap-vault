...
export class TabScanPage {
  ...
  constructor(
    private schemeService: AirGapSchemeProvider,
  ...
  transactionScanned(transaction: Transaction) {
    const correctWallet = this.secretsProvider.findWalletByPublicKeyAndProtocolIdentifier(
      ...
      transaction.protocolIdentifier
    )
    ...
    return new Promise((resolve, reject) => {
      ...
      alert.addButton({
        ...
        handler: (walletIndex: string) => {
          ...
          try {
            const tx = this.transactionProvider.constructFromPayload(
              payload,
              wallet
            )
            resolve(tx)
  ...
  simulateAirGapTx() {
    this.transactionScanned(
      this.schemeService.extractAirGapTx(
        'airgap-vault://sign?data=' +
          btoa(
            JSON.stringify({
              protocolIdentifier: 'eth',
              publicKey:
                '03c2c5da503a199294e2354425f9571d060a3a5971b4c61fcdccaf035d0fb18e6d',
              payload: {
